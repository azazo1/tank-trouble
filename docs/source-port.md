...
`just check-shield` 对照拾取护盾的坦克位置, P2 位置和屏幕位置, 覆盖移动, 缩放, 淡出及对象池复用; 加 `--headed` 检查实际显示器的像素密度. 图片桥接层保留 `Phaser.Component.Core.exists` 对可见性和 P2 世界登记的联动.

`just check-resize` 检查窗口缩放后的操作选择层, 菜单按钮, 地图有效尺寸, 玩家面板和倒计时位置. 加 `--headed` 使用实际显示器的像素密度运行调试窗口, 不生成截图. `Go` 的首轮可见帧和震屏后的屏幕坐标也纳入检查.

原生布局沿用 `ResizeManager._updateSizes` 的玩家面板高度规则和 HAR CSS 中的底部叠放关系, 游戏画布保持完整窗口尺寸. 倒计时和 HUD 在场景创建时定位, 地图尺寸计算等到迷宫创建后执行. 操作选择层和人数按钮在空间不足时整体等比缩放.
