...
const source = await Bun.file(join(root, "vendor/original/js/box2d/Box2dWeb-2.1.0-b.min.js")).text();
...
class Compiler {
  ...
  variable(name: string) {
    ...
    if (!["Array", "Math", "Number", "Object", "Function", "Error", "parseInt", "isFinite", "isNaN"].includes(name)) throw new Error(`Box2D 未绑定标识符: ${name}, 函数 ${this.scope.id}, 局部变量 ${[...this.scope.names.keys()].join(", ")}`);
  ...
  expression(node: Node): string {
    switch (node.type) {
      ...
      case "FunctionExpression": return this.temp(`r.function(box2d_function(${byNode.get(node)!.id}), env)`);
  ...
  statement(node: Node) {
    switch (node.type) {
      ...
      case "ThrowStatement": { const value = this.expression(node.argument); this.line(`throw std::runtime_error(r.text(${value}));`); break; }
      case "IfStatement": {
      ...
      case "BreakStatement": this.line("break;"); break;
      case "ContinueStatement": this.line(`goto ${this.loops.at(-1)};`); break;
  ...
  compile() {
    ...
    this.line(`Value box2d_${id}(Runtime &r, Value self, const Arguments &arguments, Environment *parent, Value callee)`);
    this.block(() => {
      ...
      for (const declaration of this.scope.declarations) this.line(`${this.variable(declaration.id.name)} = r.function(box2d_function(${byNode.get(declaration)!.id}), env);`);
      const body = node.type === "Program" ? node.body : node.body.body;
      body.forEach((statement: Node) => this.statement(statement));
      this.line(`return ${node.type === "Program" ? this.variable("Box2D") : "Value()"};`);
    });
    return this.lines.join("\n");
  }
}

const target = join(root, "build/original-box2d");
...
for (let start = 0; start < scopes.length; start += 32) {
  ...
  await output(join(target, `box2d-${start}.cpp`), common + functions.join("\n\n") + "\n}\n");
  log.info({ compiled: Math.min(start + 32, scopes.length), total: scopes.length }, "Box2D 原函数已转译为 C++");
}
const declarations = scopes.map(({ id }) => `Value box2d_${id}(Runtime &, Value, const Arguments &, Environment *, Value);`).join("\n");
const table = `Function box2d_function(size_t index) { static Function functions[] = {${scopes.map(({ id }) => `box2d_${id}`).join(", ")}}; return functions[index]; }`;
await output(join(target, "box2d-index.cpp"), common + declarations + "\n" + table + `\nint box2d_function_id(Function function) { for (int i = 0; i < ${scopes.length}; ++i) if (box2d_function(i) == function) return i; return -1; }` + "\nValue initialize_box2d(Runtime &runtime) { return box2d_0(runtime, Value(), {}, nullptr, Value()); }\n}\n");
await output(join(target, "source-map.json"), JSON.stringify({ source: "vendor/original/js/box2d/Box2dWeb-2.1.0-b.min.js", functions: scopes.map(({ id, node }) => ({ id, start: node.start, end: node.end })) }, null, 2) + "\n");
